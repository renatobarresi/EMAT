...
{
	volumeInit,       // Physical layer initialized correctly
	volumeNotInit,	  // Error initializing the physical layer
    FileSystemOk,     // filesystem is in a good state
    FileSystemError,  // there was a general filesystem error
    MountError,       // there was an error while mounting the filesystem
    UnmountError,     // there was an error while unmounting the filesystem
    ReadError,        // there was an error while reading from the filesystem
    WriteError,       // there was an error while writing to the filesystem
    FormatError,      // there was an error while formatting the filesystem
    SpaceError        // there is insufficient space in the filesystem
};
